...
class Fish extends Component {
  ...
  setFish = (result) => {
    this.setState({allFish: result});
    this.setCaughtFish(result);
  };

  setCaughtFish = () => {
    const setCF = this.state.allFish.filter(
      (oneFish) => oneFish.caught !== false
    );
    this.setState({
      caughtFish: setCF,
    });
  ...
  showFish = () =>
    this.state.allFish.map((oneFish) => {
      return (
        <FishCard
          key={oneFish.id}
  ...
  addToCaught = (oneFish) => {
    if (
      !this.state.caughtFish.find((activeFish) => oneFish.id === activeFish.id)
    ) {
      this.setState({
  ...
  showCaught = () =>
    this.state.caughtFish.map((oneFish) => {
      return (
        <FishCard
          key={oneFish.id}
  ...
  removeFromCaught = (oneFish) => {
    const filtered = this.state.caughtFish.filter(
      (activeFish) => activeFish.id !== oneFish.id
    );
    this.setState({caughtFish: filtered});
  };
      ...
      <div className='main'>
        ...
        <div className='all-fish'>
          <div className='fishContainer'>
            ...
            <ul className='fish-list'>{this.showFish()}</ul>
          </div>
          <div className='caught-fishContainer'>
            <h2 className='CaughtTitle'>CAUGHT</h2>
            <ul className='caught-list'>{this.showCaught()}</ul>
          </div>
        </div>
      </div>
    );
